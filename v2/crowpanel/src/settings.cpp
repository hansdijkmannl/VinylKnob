...
void settingsLoad() {
  ...
  strlcpy(settings.brainHost, prefs.getString("brain", "").c_str(), sizeof(settings.brainHost));
...
void settingsSave() {
  ...
  prefs.putString("brain", settings.brainHost);
...
void settingsToJson(String &out) {
  ...
  doc["brainHost"]      = settings.brainHost;
...
bool settingsFromJson(const String &body, String &err, bool &wifiChanged) {
  ...
  if (doc["brainHost"].is<const char *>())
    strlcpy(settings.brainHost, doc["brainHost"], sizeof(settings.brainHost));
