...
bool shelfLoad(const char *host, uint16_t port) {
  ...
  char url[96];
  snprintf(url, sizeof(url), "http://%s:%u/shelf", host, port);

  HTTPClient http;
  http.setConnectTimeout(1000);
  http.setTimeout(8000);                 // 25 kB over Wi-Fi may take a moment
  if (!http.begin(url)) return false;
  if (http.GET() != HTTP_CODE_OK) { http.end(); return false; }

  const int length = http.getSize();
  if (length <= 0 || length > 512 * 1024) { http.end(); return false; }
...
static bool fetchArtwork(const char *host, uint16_t port, int album) {
  ...
  snprintf(url, sizeof(url), "http://%s:%u/shelfcover?id=%u&px=%d",
           host, port, (unsigned)albums[album].id, SHELF_PX);
