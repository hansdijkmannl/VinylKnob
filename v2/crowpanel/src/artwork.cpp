...
// and brightness, then that hue re-emitted at a fixed saturation and
// brightness — so the arc is always bright enough to stand out against the
// sleeve.
static void findAccent() {
  ...
  for (int y = 0; y < ARTWORK_PX; y += step) {
    for (int x = 0; x < ARTWORK_PX; x += step) {
      ...
      weight[bucket] += (d / mx) * mx;               // saturation x brightness
    }
  }
...
static bool haalEnDecodeer(const char *url) {
  ...
  if (length <= 0 || (size_t)length > JPEG_MAX) {
    Serial.printf("[artwork] length %d does not fit (max %u)\n", length, (unsigned)JPEG_MAX);
    http.end();
    return false;
  }
...
bool artworkLoad(const char *host, uint16_t port) {
  ...
  char url[96];
  snprintf(url, sizeof(url), "http://%s:%u/artwork", host, port);
  return haalEnDecodeer(url);
}

bool artworkLoadAlbum(const char *host, uint16_t port, uint16_t releaseId) {
  if (!pixels || !jpeg) return false;
  if (WiFi.status() != WL_CONNECTED) return false;
  // The same size /artwork delivers, because it ends up in the same place:
  // full-screen behind the volume.
  char url[112];
  snprintf(url, sizeof(url), "http://%s:%u/shelfcover?id=%u&px=%d",
           host, port, (unsigned)releaseId, ARTWORK_PX);
  return haalEnDecodeer(url);
}
